...
use "${data}/${sitename}_RawCensoredEpisodes_calendar_st_ready", clear
local splits = ""
...
save "${data}/${sitename}_RawCensoredEpisodes_month_split", replace
export delimited using "${data}/${sitename}_RawCensoredEpisodes_month_split.csv", replace

//split by year, month and age group
use "${data}/${sitename}_RawCensoredEpisodes_month_split", clear
drop _st _d _t _t0 exposure_days 
stset enddate, id(individualid) fail(died) enter(exposure_beg) origin(dob) 
save "${data}/${sitename}_RawCensoredEpisodes_month_split_st_age_ready", replace

stsplit age_days, at(0(365.25)36525)
//create standard WHO  age groups
gen age_yrs = age_days/365.25
recode age_yrs (0/4=0) (5/14=5) (15/49=15) (50/64=50) ///
...
save "${data}/${sitename}_RawCensoredEpisodes_month_age_split", replace
...
//export to excel
quietly export excel using "${output}/${sitename}.xlsx", sheet("py_mx_month_age")sheetreplace firstrow(variables)
...
//export to excel
quietly export excel using "${output}/${sitename}.xlsx", sheet("py_mx_month")sheetreplace firstrow(variables)
...
//mortality rates by month, year, age group and sex
use "${data}/${sitename}_RawCensoredEpisodes_month_age_split", clear
strate year month age_group sex, output("${data}/${sitename}_py_mx_month_age_sex",replace) nolist
use "${data}/${sitename}_py_mx_month_age_sex", clear
format _Y %9.0g
...
gen ub_mx_1000 = Upper95Mx *1000 *365.25
gen centreid = "${sitename}"
save "${data}/${sitename}_py_mx_month_age_sex",replace
//export to excel
quietly export excel using "${output}/${sitename}.xlsx", sheet("py_mx_month_age_sex")sheetreplace firstrow(variables)
tab age_group if Lower95Mx==. & year <2022
